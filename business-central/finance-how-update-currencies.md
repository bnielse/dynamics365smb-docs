...
> [!NOTE]  
> When you post to G/L using a currency code, for example to post an expense in a general journal using a currency code, the transaction is converted to LCY using the currency exchange rate for the posting date. The G/L entry(/-ies) will not contain information of which currency was used, only it's value in LCY.
